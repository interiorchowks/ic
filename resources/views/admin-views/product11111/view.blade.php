    ...
    <div class="content container-fluid" style="text-align: {{ Session::get('direction') === 'rtl' ? 'right' : 'left' }};">
        ...
        <div class="d-flex align-items-center justify-content-between flex-wrap gap-10 mb-3">
            ...
            <div class="d-flex justify-content-end flex-wrap gap-10">
                ...
                <a class="btn btn--primary" data-toggle="modal" onclick="services_tags({{ $product['id'] }})"
                    data-target=".bd-example-modal-lg">{{ \App\CPU\translate('service_tags') }}</a>
                <a href="{{ url()->previous() }}" class="btn btn--primary">
                    <i class="tio-back-ui"></i> {{ \App\CPU\translate('Back') }}
                </a>
                <a href="{{ route('product', $product['slug']) }}" class="btn btn--primary " target="_blank"
                    style="display:none;"><i class="tio-globe"></i> {{ \App\CPU\translate('View') }}
                    {{ \App\CPU\translate('from') }} {{ \App\CPU\translate('Website') }}
                </a>
            </div>
        </div>
                    ...
                    <div class="col-md-auto mb-3 mb-md-0">
                        <div class="d-flex align-items-center">
                            <img class="avatar avatar-xxl avatar-4by3 {{ Session::get('direction') === 'rtl' ? 'ml-4' : 'mr-4' }}"
                                ...
                                src="{{ rtrim(env('CLOUDFLARE_R2_PUBLIC_URL'), '/') . $product['thumbnail'] }}"
                            ...
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($sku as $index => $item)
                                        @php($sellingPrice = $item->variant_mrp - ($item->discount ?? 0))
                                        <tr>
                                            <td>{{ $index + 1 }}</td>
                                            <td>{{ $item->sku }}</td>
                                        ...
                                        <a class="d-flex align-items-center"
                                            ...
                                            <div class="avatar avatar-circle">
                                                <img class="avatar-img"
                                                    ...
                                                    src="{{ rtrim(env('CLOUDFLARE_R2_PUBLIC_URL'), '/') . $review->customer->image}}"
                                                    alt="Image Description">
                                            </div>
        ...
        function saveServicesTags() {
            ...
            $.ajax({
                ...
                success: function() {
                    ...
                    toastr.success(
                        '{{ \App\CPU\translate('
                                                                                                                Services Tags updated successfully!') }}'
                    );
                }
            });
        }
