...
class ProductController extends Controller
{
    ...
    public function remove_image(Request $request)
    {
        ...
        $imagePath = 'products/' . $request->name;
...
public function bulk_export_data_category_wise()
{
    ...
    $seller_id = auth('seller')->id();
    $commission_fee = DB::table('categories')->where('id', $category_id)->first();
    ...
    // Repeat some common fields at the end (as in your original)
    $baseData = array_merge($baseData, [
        'Return Days'            => '',
        'Replacement Days'       => '',
        'Unit'                   => '',
    ...
    $map = [
        ...
        visibility'],
        'MOQ' => ['Number', 'Write a minimum order quantity'],
        'Packaging Dimensions' => [['Length (CM)','Breadth (CM)','Height (CM)','Weight (KG)'], [
    ...
    // Build Sheet 1 rows + merge config (for header colspans)
    $sheet1Rows = [];
    $mergeConfig = [];

    // Row 1: Headers (with merge info)
    $row1 = [];
    $colIndex = 1; // Excel columns start from 1 (A)

    foreach ($keys as $key) {
        if (isset($map[$key]) && is_array($map[$key][0])) {
            $subCount = count($map[$key][0]);

            if ($subCount > 1) {
                $mergeConfig[] = [
                    'start' => $colIndex,
                    'end'   => $colIndex + $subCount - 1,
                ];
            }
    ...
    // Sheet 2: simple Guideline dummy data
    $sheet2Rows = [
        ['Field', 'Guideline'],
        ['Seller SKU ID', 'Unique id per product variant, max 64 chars.'],
        ['Listing Status', 'Keep Active to show product to buyers.'],
        ['MRP (INR)', 'Maximum retail price inclusive of all taxes.'],
        ['Discount Type', 'Use Flat or Percent.'],
        ['Your selling price (INR)', 'Auto-calculated from MRP & Discount.'],
        ['Colour', 'Use standard colour names, avoid typos.'],
        ['Size', 'Use S/M/L or clear custom size.'],
        ['Images', 'High quality, white/clean background, proper angles.'],
        ['Video URL', 'Public video links only.'],
        ['PDF URL', 'Public PDF spec sheets / brochures.'],
        ['HSN & Tax', 'Will be verified by InteriorChowk team.'],
        ['Stock / MOQ', 'Maintain realistic stock and MOQ to avoid cancellations.'],
    ];

    return Excel::download(
        new class($sheet1Rows, $sheet2Rows, $mergeConfig) implements WithMultipleSheets {

            public function __construct(
                protected array $sheet1Rows,
                protected array $sheet2Rows,
            ...
            {
                return [
                    // Sheet 1: product_bulk
                    new class($this->sheet1Rows, $this->mergeConfig)
                        implements FromArray, WithTitle, WithEvents, WithStyles {
                            ...
                            return [
                                ...
                                1 => [
                                    ...
                                    'fill' => [
                                        ...
                                        'startColor' => ['argb' => 'FFC4D79B'], // #c4d79b
                                    ],
                                ],
                                // Row 2: Yellow
                                2 => [
                                    ...
                                    'fill' => [
                                        'fillType' => Fill::FILL_SOLID,
                                        'startColor' => ['argb' => 'FFFFFF00'], // #ffff00
                                    ],
                                ],
                                ...
                                3 => [
                                    ...
                                    'fill' => [
                                        'fillType' => Fill::FILL_SOLID,
                                        'startColor' => ['argb' => 'FFFCD5B4'], // #fcd5b4
                                    ],
                                ],
                            ];
                        }
                        ...
                        public function registerEvents(): array
                        {
                            return [
                                AfterSheet::class => function (AfterSheet $event) {
                                    ...
                                    // 1) Merge header cells for row 1 based on $this->merges
                                    foreach ($this->merges as $merge) {
                                        $startCol = $this->columnLetter($merge['start']);
                                        $endCol   = $this->columnLetter($merge['end']);
                                    ...
                                    // 2) Set fixed width for ALL columns
                                    //    (change 25 to 20 / 30 as per your preference)
                                    ...
                                    // 3) Set row heights for header rows
                                    $sheet->getRowDimension(1)->setRowHeight(25);
                                    $sheet->getRowDimension(2)->setRowHeight(35);
                                    $sheet->getRowDimension(3)->setRowHeight(35);

                                    // 4) Wrap text for ALL used cells
                                    $highestRow = $sheet->getHighestRow();
                                    $highestColumnLetter = $this->columnLetter($colCount);
                    ...
                    // Sheet 2: Guideline
                    new class($this->sheet2Rows) implements FromArray, WithTitle {
                        public function __construct(protected array $rows) {}
                        public function array(): array { return $this->rows; }
                        public function title(): string { return 'Guideline'; }
                    },
                ];
            }
        },
        'products_bulk_with_guideline.xlsx'
    );
}
...
//     // Sheet 2: simple Guideline dummy data (same data you had)
//     $sheet2Rows = [
//         ['Field', 'Guideline'],
//         ['Seller SKU ID', 'Unique id per product variant, max 64 chars.'],
//         ['Listing Status', 'Keep Active to show product to buyers.'],
...
//             {
//                 return [
//                     // Sheet 1: product_bulk (UNCHANGED)
//                     new class($this->sheet1Rows, $this->mergeConfig)
//                         implements FromArray, WithTitle, WithEvents, WithStyles {
...
//                     // Sheet 2: Guideline – Amazon-style layout with images
//                     new class($this->sheet2Rows)
//                         implements FromArray, WithTitle, WithStyles, WithEvents, WithDrawings {

//                         public function __construct(protected array $rows) {}

//                         public function array(): array
//                         {
//                             // Same data you defined in $sheet2Rows
//                             return $this->rows;
//                         }
...
//                                     'fill' => [
//                                         'fillType'   => Fill::FILL_SOLID,
//                                         'startColor' => ['argb' => 'FF7F7F7F'], // dark grey
//                                     ],
//                                 ],
//                             ];
//                         }
...
//                                     // Column widths like Amazon Images sheet:
//                                     // A = guideline text, B = image / example text
...
//                                     if ($highestRow >= 2) {
//                                         $dataRange = "A2:B{$highestRow}";

//                                         // Light green background + top-left text + wrap
...
//                         /**
//                          * Images in Guideline sheet – adjust paths & rows as per your files
//                          */
//                         public function drawings(): array
//                         {
//                             $drawings = [];

//                             // Map: row number => local image path
//                             // TODO: yahan apne actual image paths set karo
//                             $images = [
//                                 // Example:
//                                 3 => "https://pub-3593718b2c3a49558e703e35d10e7897.r2.dev/mail/ytlogo.png",
//                                 4 => "https://pub-3593718b2c3a49558e703e35d10e7897.r2.dev/mail/ytlogo.png",
//                                 5 => "https://pub-3593718b2c3a49558e703e35d10e7897.r2.dev/mail/ytlogo.png",
//                             ];

//                             foreach ($images as $row => $path) {
//                                 if (! file_exists($path)) {
//                                     continue;
//                                 }

//                                 $drawing = new Drawing();
//                                 $drawing->setName('Guideline Example '.$row);
//                                 $drawing->setDescription('Guideline Example '.$row);
//                                 $drawing->setPath($path);
//                                 $drawing->setHeight(90); // adjust height as needed
//                                 $drawing->setCoordinates('B'.$row); // place in column B
