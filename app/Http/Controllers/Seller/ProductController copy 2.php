...
public function bulk_export_data_category_wise()
{
    ...
    //         'Discount Type'              => '',
    //         'Discount'                   => '',
    //         'Your selling price (INR)'   => '', // row-wise formula later
    //         'Product Title'              => '',
    //         'Product Description'        => '',
    //         'Colour'                     => implode(', ', $colors),
    //         'Size'                       => '',
    ...
    //             $set(
    //                 'Catelogue QC Status',
    //                 is_null($p->qc_failed_reason) ? 'Approved' : 'Rejected'
    //             );
    ...
    //             $set('Your selling price (INR)', "=IF(M{$rowIndex}=\"Flat\", MAX(0, L{$rowIndex}-N{$rowIndex}), IF(M{$rowIndex}=\"Percent\", MAX(0, L{$rowIndex}-(L{$rowIndex}*N{$rowIndex}/100)), \"\"))");
    ...
    //             // grouped: description + bullets
    //             $setDim('Description', strip_tags($p->details ?? ''));
    ...
    //     // data style + autosize
    //     $sheet1->getStyle("A{$dataStart}:{$lastColLetter}".max($dataStart, $rowIndex-1))->applyFromArray([
    //         'alignment' => [
    //             'horizontal' => \PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER,
    //             'vertical'   => \PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER,
    //             'wrapText'   => true,
    ...
    public function bulk_export_data()
    {
        ...
        foreach ($products as $p) {
            ...
            foreach ($variants as $v) {
                ...
                // NOTE: This formula is using fixed columns (L/M/N). If your template column order changes, update this.
                $set(
                    'Your selling price (INR)',
            ...
            // if (!file_exists($tmpOut) || filesize($tmpOut) <= 0) { $failed++; Log::error("BulkImage: tmpOut not // //
                ...
                for ($row = 2; $row <= $highestRow; $row++) { $processed++; $idCell=trim((string) $sheet->
                    ...
                    if (!$resp->successful() || !file_exists($tmpIn) || filesize($tmpIn) <= 0) { $failed++;
                        @unlink($tmpIn); continue; } // resize with padding 1000x1000 $img=Image::make($tmpIn)->
                        ...
                        if (!file_exists($tmpOut) || filesize($tmpOut) <= 0) { $failed++; @unlink($tmpIn);
                            @unlink($tmpOut); continue; } $base=basename(parse_url($url, PHP_URL_PATH) ?? 'image' );
                            $base=preg_replace('/[^a-zA-Z0-9\-_\.]/', '_' , $base); $base=Str::limit($base, 60, '' );
                            $base=preg_replace('/\.(jpg|jpeg|png|webp)$/i', '' , $base); $rand=Str::random(6);
                            $r2Path="products/lists/{$sellerId}_{$base}_{$rand}.webp" ; $stream=fopen($tmpOut, 'r' );
                            Storage::disk('r2')->writeStream($r2Path,
                            $stream,
                            [
                            'visibility' => 'public',
                            'mimetype' => 'image/webp',
